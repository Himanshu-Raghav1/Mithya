...
export default function QRGenerator() {
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `Mithya_QR_${Date.now()}.svg`;
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto space-y-6 pb-24">
      ...
      <div className="glass-card p-5 border border-purple-500/30 flex items-center gap-4" style={{ background: 'rgba(147, 51, 234, 0.1)' }}>
        ...
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-white">Mithya QR Forge</h2>
          <p className="text-white/80 text-sm mt-0.5 leading-snug">
            Mithya will make a QR for your Website, Portfolio, Any Link or Secret Message for <span className="font-bold text-yellow-400">FREE</span>.
          </p>
        </div>
      </div>
      ...
      <div className="flex bg-black/40 p-1 rounded-xl border border-white/10">
        ...
        >
          <LinkIcon className="w-4 h-4" /> Link Mode
        </button>
        <button 
        ...
        >
          <MessageSquare className="w-4 h-4" /> Message Mode
        </button>
      </div>
      ...
      {inputValue.trim() && (
        ...
        >
          ...
          <div className="bg-white p-4 rounded-2xl shadow-2xl z-10">
            <QRCodeSVG 
              value={finalQRValue} 
              size={200}
              level="H"
