...
// Doraemon Reading SVG - Used for Banner
...
export default function PYQsNotes() {
  ...
  return (
    <div className="p-4 space-y-4 max-w-3xl mx-auto pb-24">
      {/* Banner */}
      <div className="glass-card p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-4 border" style={{ background: 'rgba(255,215,64,0.12)', borderColor: 'rgba(255,215,64,0.3)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {isLoading ? (
            ...
            <div className="text-center py-16 opacity-50">
              ...
              <p className="text-white">No verified notes strictly available yet.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {notes.map(note => (
                <div key={note.id} className="glass-card p-5 border border-white/10 hover:border-doraSky transition-colors group">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="text-xs text-white/30">{new Date(note.timestamp).toLocaleDateString()}</span>
                  </div>
                  <h3 className="text-lg font-black text-white mb-4 line-clamp-2">{note.title}</h3>
      ...
      {activeTab === 'upload' && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 border border-white/10">
          <form onSubmit={handleSubmitNote} className="space-y-4">
            ...
            <button disabled={isUploading} type="submit" className="w-full py-3.5 mt-6 rounded-xl font-black text-[15px] flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-400 text-black transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-50">
              {isUploading ? <><Loader2 className="w-5 h-5 animate-spin" /> Uploading securely...</> : "Submit for Verification"}
            </button>
            <p className="text-center text-[10px] text-white/40 mt-2">All uploads are subject to Admin review before appearing on the feed.</p>
          </form>
        </motion.div>
      )}
    </div>
